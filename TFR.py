...
def test_records(filename):
    ...
    with tf.Session() as sess:
         ...
         for i in range(5):
              ...
              label = tf.sparse_to_dense(label, [len(label), 64], 1)
              print val.shape, label.shape

def fetchData_v1(filename, b_size, mode=1):
...
def fetch_data(filename, b_size, one_hot=True):
    image_batch, label_batch = read_and_decode(filename, b_size)
    global num_class
    with tf.Session() as sess:
         sess.run(tf.global_variables_initializer())
         ...
         img, label = sess.run([image_batch, label_batch])
         if one_hot:
              # label = tf.sparse_to_dense(label, [len(label), num_class], 1)
...
if '__main__' == __name__:
    ...
    img, label = fetch_data('train.tfrecords',100)
    print label
    # test_records('train.tfrecords')
